...
async function fetchArrayBuffer(url) {
  ...
  class AudioController {
     ...
     * @return {Promise<AudioController>}
     */
    async initialize(latencyHint = 'interactive', volume = 0.8) {
      ...
      this.buffer = await this.context.decodeAudioData(this.undecodedBuffer.slice());
     ...
     */
    start() {
      if (this.started || !this.initialized || this.context.state === 'closed') return this;
    ...
    analyzeFrequency(
      ...
      let { fftSize = undefined, minFrequency = undefined, maxFrequency = undefined } = options;
      ...
      const endIndex = Math.min(Math.floor((maxFrequency / sampleRate) * fftSize), bufferLength);
